...
const productPatch = productCretae.partial();
...
export async function createAdminProduct(
  ...
  try {
    const parsed = productCretae.safeParse(req.body);
    if (!parsed.success) {
      res
        .status(400)
        .json({ error: "Invaild bool", detail: parsed.error.flatten() });
      return;
    }
