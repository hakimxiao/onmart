...
const CatalogProductCard = ({ product }) => {
  ...
  return (
    <article className="card group h-full overflow-hidden border-base-300 bg-base-100 shadow-md transition hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-xl">
      <Link
      ...
      <div className="card-body grow gap-3 p-5 text-left">
        <Link
          to={`/product/${product.slug}`}
          className="card-title line-clamp-2 transition group-hover:text-primary"
        >
          {product.name}
        </Link>
