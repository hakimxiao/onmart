...
export function OrderPreview({ items, size = "md" }) {
  ...
  if (!items?.length) {
    return (
      <div
        className={`flex shrink-0 items-center justify-center rounded-2xl border border-dashed border-base-300 bg-base-200/50 ${box}`}
      >
        <PackageIcon
          className={
            size === "lg"
    ...
    return (
      ...
      >
        {p.imageUrl ? (
          ...
          <div className="flex h-full items-center justify-center bg-linear-to-br from-base-300 to-base-200">
            <PackageIcon
              className={
                size === "lg"
                  ? "size-12 text-base-content/25"
    ...
    >
      {show.map((p, i) => (
        ...
        >
          ...
          {i === cap - 1 && rest > 0 ? (
            <div className="absolute inset-0 flex items-center justify-center bg-neutral/90 text-sm font-black tabular-nums text-neutral-content">
              +{rest}
            </div>
          ) : null}
        </div>
